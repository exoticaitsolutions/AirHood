...
console.log('Klaviyo tracking working');
...
(function($) {
  ...
  function show_klaviyo_emdeb_phone_input($input) {
    //$input.closest('.klaviyo-embed-form-wrapper').find('form').find('[data-testid="form-row"]').slideDown( "800");
  }
    ...
    for(const mutation of mutationsList) {
      ...
      if ( $(mutation.target).find('button').length > 0
        ...
        setTimeout(function(){
         ...
         // $form.find("p").closest('[data-testid="form-row"]').css('display', "none");
...
/* We wait 2 sec since the forms are embed from klaviyo */
/*setTimeout(function(){
    $('.klaviyo-form').each(function(index) {

      var form = $(this).find('form'),
          id = $(this).attr('id');

        // if not defined is footer form, the others have the container id defined
        if (undefined == id) {
          id = 'footer_email_capture';
        }
        form.attr('id', id);
    ...
    var $this = $( ".klaviyo-form form input[type=email]" );
    $this.on('paste keyup focus', function() {
      show_klaviyo_emdeb_phone_input($this);
    });

    $('form [data-testid=form-row]:first-child  > [data-testid="form-component"] > button').on("click", function(e) {
      form_id_clicked = $(this).closest('form').attr('id');
    });
  ...
  */
/* if we have issues with 2 sec timeouts, we can try this solution:
  $('.widget_klaviyo_embedemailsignup_widget').on('DOMNodeInserted', function(){

    if ($( ".klaviyo-form form input[type=email]" ).length > 0) {
      console.log('footer form updated');
      console.log($( ".klaviyo-form form input[type=email]" ).val());
    } else {
      console.log('modified not email still');
    }
  });
*/
